...
export class FlashbotsBundleProvider extends providers.JsonRpcProvider {
  ...
  async sendRawBundle(
    ...
    const bundleTransactions = signedBundledTransactions.map(
      (signedTransaction) => {
        const transactionDetails = ethers.utils.parseTransaction(
          signedTransaction
        );
        return {
          signedTransaction,
      ...
      const handler = async (blockNumber: number) => {
        ...
        if (blockNumber < targetBlockNumber) {
          ...
          const bundleIncluded = transactionAccountNonces.every(
            (transaction, i) =>
              block.transactions[block.transactions.length - 1 - i] ===
              transaction.hash
          );
          resolve(
            bundleIncluded
  ...
  simulate(
    ...
    >,
    targetBlockNumber: number
  ): Promise<Array<SimulationResponse>> {
    // TODO simulate
    console.error(
      "Running simulation on " +
        bundledTransactions.length +
        " transactions on blockNumber " +
        targetBlockNumber
    );
